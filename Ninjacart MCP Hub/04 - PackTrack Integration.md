...
- Embedding provider: **Voyage AI `voyage-4`** (1024 dims) — switched from the initial OpenAI choice on 2026-08-14 (200M free tokens, better retrieval quality). Also uses Voyage's `inputType` (`document`/`query`) to optimize each side of a match. `src/knowledge/embed.js` isolates this choice to one file. Voyage's free tier caps requests at 3/min without a payment method — `embed.js` retries with backoff on 429s.
